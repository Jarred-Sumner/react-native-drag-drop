...
class DragModal extends React.Component {
  render() {
    ...
    return <Modal transparent={true} supportedOrientations={allOrientations} onRequestClose={() => {}}>
    ...
    componentWillMount() {
      this._panResponder = PanResponder.create({
        onStartShouldSetPanResponderCapture: () => (
          /* Helps with an issue on devices with poor touchscreens where the
           * drag action gets "stuck" if the user lifts their finger immediately
           * after triggering it.  Adding this code makes the pan responder kick
           * in and reset the state via onPanResponderRelease callback
           */
        ...
        onMoveShouldSetPanResponder: (evt, gestureState) => !!this.state.draggingComponent && gestureState.dy > gestureState.dx,
